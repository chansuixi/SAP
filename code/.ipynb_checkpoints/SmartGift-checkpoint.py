...
### funclist:所有的合约的类型和名称
### input_type_list : 目标函数对应的输入
### input_list :所有合约对的输入
...
def process_functions(funclist, input_type_list, input_list, graph_list, k):
    ...
    for i in range(len(funclist)):
        ...
        # 这么做的目的是利用了，数组的有序特性，从而避免实用二元数组不方便处理
...
def trim_func(func_list, input_type_list, input_list,graph_list, k):
    ...
    # 优先判断图结构是否相同，不相同直接跳过first_level
        ...
        for input_type in input_type_list:
            ...
            for j in range(len(type_json)):
                ...
                if func_type == input_type and (j not in have_set):
                    ...
                    have_set.append(j)
                    break
        if types_are_all_exist == len(input_type_list): #类型完全相等
            second_level.append(func_list[i])
            second_level_input.append(input_list[i])
        if (types_are_all_exist != len(input_type_list)) or len(graph_json) > 0 and len(graph_list) > 0 and graph_json == graph_list: # 因为只提取了合约图，普通的流程关系不会走到这，一定是运行流程差不多的
            have_set_temp = []
...
def train_and_get_similarity(wordlist, test):
    ...
    # model_name = 'bert-base-multilingual-cased'  # SMARTGIFT 做时间对比用
    ...
    # sample_vec = np.vstack([encode_text(item) for item in wordlist])
    #
    # # 重塑test_vec以匹配sample_vec的形状
    # test_vec = test_vec.reshape(1, -1)
    #
    # sim = cosine_similarity(sample_vec, test_vec)
    # final_sim = [item[0] for item in sim]
    #
    # return final_sim
...
def main():
    ...
    filepath = "/Users/yangkaixuan/Downloads/SmartGift-main/code/test/0-200/1_v0.4.11_0x5d0e1a73ec9f0c5c13d43f8315674855ed2b3d4e.abi"
    sourceCodePath = "/Users/yangkaixuan/Downloads/SmartGift-main/code/test/0-200/1_v0.4.11_0x5d0e1a73ec9f0c5c13d43f8315674855ed2b3d4e.sol"
    samplepath = "/Users/yangkaixuan/Downloads/SmartGift-main/code/crawl/samples/testforconstant/sample1.json"
    outputpath = "/Users/yangkaixuan/Downloads/SmartGift-main/code/test" # output file path
    # k = int(sys.argv[4]) #topk
    k = 5
    # wordlist : 所有的函数名 input_list：所有的输入内容
    ...
    for item in target_functions:
        ...
        # final_word_list 筛选后的函数签名
        # final_func_list 筛选后后的函数列表word_list子集
        # final_input_list 筛选后的函数输入
        # level R集合的级别
        ...
        # sim_matrix_2 = train_and_get_similarity(final_param_list, )
