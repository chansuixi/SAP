...
fd = open('FuzzingwithState/code/crawl/samples/testforconstant/all_funcs_need1.json')
content = fd.read()
...
for i in range(len(sample_content)):
	...
	for item in cur_func['function']['inputs']:
		signature += ' ' + item['type']
	for item in cur_func['graph'].split(","):
		signature += ' ' + item
...
for item in sample_dic.keys():
	...
	for funcs in l:
		...
		if isFirst:
			...
			one_func['function']['graph'] = funcs['graph']
			one_func['function']['param'] = funcs['param']
...
print(len(res))
fd = open('FuzzingwithState/code/crawl/samples/testforconstant/sample1.json', 'w+')
